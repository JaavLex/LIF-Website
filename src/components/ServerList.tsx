  ...
  const fetchServers = async () => {
    try {
      ...
      if (data.servers && data.servers.length > 0) {
        ...
        setServers(fallbackServers.map(s => ({
          name: s.name,
          mode: s.mode,
      ...
      <div className="section-container">
        ...
        {lastUpdate && (
          <p className="servers-update">
            ...
            <button onClick={fetchServers} className="refresh-btn" disabled={loading}>
          ...
          {loading && servers.length === 0 ? (
            ...
            servers.map((server, index) => (
              <div key={index} className="server-card">
                <div className={`server-status ${server.isOnline ? 'online' : 'offline'}`}></div>

...
const STATIC_TEXTS = [
	'FREQ 148.250 MHz', 'ENC AES-256', 'LINK 16 UP', 'SAT COM III',
	'QRF EN ATTENTE', 'ROE ALPHA', 'UTC+01:00', 'DEFCON 3',
	'COMMS CHECK OK', 'SIGINT ACTIF', 'NET STATUS ✓', 'STANDBY',
	'48°51\'N 2°21\'E', '43°17\'N 5°22\'E', 'GRID 12S PA 456 789',
	'ALT 320M', 'BRG 047°', 'RNG 1.2KM', 'AO SERPENT',
	'OPORD 2026-04', 'SITREP EN ATTENTE', 'RTLS 0.87',
	'CONTRAT #4471', 'PAIEMENT VALIDÉ', 'FONDS OPS: ██████',
	'LIVRAISON CONFIRMÉE', 'ETA 0430Z', 'LZ BRAVO SÉCURISÉ',
	'PRIORITÉ: HAUTE', 'CANAL SÉCURISÉ', 'CRYPTO ACTIF',
];
...
export default function MatrixBackground() {
	...
	const generateTextFromData = useCallback((): { text: string; color: { r: number; g: number; b: number } } => {
		...
		if (roll < 0.4 && chars.length > 0) {
			...
			const variants = [
				{ text: `${c.militaryId || 'N/A'} — ${c.lastName?.toUpperCase()} ${c.firstName}`, color: { r: 0, g: 255, b: 65 } },
				{ text: `STATUT: ${STATUS_FR[c.status] || c.status?.toUpperCase()} [${c.faction || '?'}]`, color: { r: 0, g: 255, b: 65 } },
				{ text: `${c.rank || 'N/A'} ${c.lastName?.toUpperCase()} — ${c.faction || 'INCONNU'}`, color: { r: 0, g: 255, b: 65 } },
			];
			if (c.isTarget) {
				variants.push(
					{ text: `◆ CIBLE: ${c.lastName?.toUpperCase()} — MENACE ${THREAT_FR[c.threatLevel || ''] || '?'}`, color: { r: 255, g: 80, b: 50 } },
					{ text: `TGT ${c.lastName?.toUpperCase()} / ${c.targetFaction?.toUpperCase() || '?'} — ACQ`, color: { r: 255, g: 80, b: 50 } },
				);
			}
		...
		if (roll < 0.6 && intels.length > 0) {
			...
			const variants = [
				{ text: `RENS: ${intel.title?.toUpperCase().slice(0, 35)}`, color: { r: 100, g: 180, b: 255 } },
				{ text: `${INTEL_TYPE_FR[intel.type] || intel.type?.toUpperCase()} — ${intel.coordinates || 'COORD N/A'}`, color: { r: 100, g: 180, b: 255 } },
			];
			if (intel.classification === 'classified') {
				variants.push({ text: `[CLASSIFIÉ] ${intel.title?.toUpperCase().slice(0, 28)}`, color: { r: 255, g: 50, b: 30 } });
			}
			return variants[Math.floor(Math.random() * variants.length)];
		}
	...
	const drawDossier = useCallback((
		...
		if (d.type === 'character') {
			...
			ctx.fillText(isTarget ? '◆ CIBLE / TARGET ◆' : '■ DOSSIER PERSONNEL', x + pad, y + 15);
			...
			if (isTarget) {
				...
				if (c.threatLevel) {
					...
					ctx.fillStyle = c.threatLevel === 'critical'
